...
export default function AboutPage() {
  ...
  const heroImage = settings.aboutHeroImageUrl || settings.heroImageUrl || 'https://images.unsplash.com/photo-1490481651871-ab68de25d43d?auto=format&fit=crop&w=2000&q=90';
  const craftsmanshipImage = settings.aboutCraftsmanshipImageUrl || 'https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?auto=format&fit=crop&w=1000&q=80';
  const pillars = settings.aboutPillars && settings.aboutPillars.length > 0 ? settings.aboutPillars : [
    {
  ...
  return (
    <div className="space-y-20 pb-20 animate-fade-in">
      ...
      <section className="relative py-24 bg-noir-900 border-b border-gold-600/20 overflow-hidden">
        <div className="absolute inset-0 opacity-20 pointer-events-none">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-t from-noir-900 via-noir-900/80 to-transparent" />
        </div>
        ...
        <div className="relative max-w-4xl mx-auto px-4 text-center space-y-4">
          ...
          <h1 className="font-serif text-4xl sm:text-6xl text-neutral-100 font-light gold-gradient-text">
            {settings.aboutHeroTitle || `The World of ${settings.companyName || 'Annex Couture'}`}
          </h1>

          <p className="text-xs sm:text-sm text-neutral-300 max-w-2xl mx-auto font-light leading-relaxed">
            {settings.aboutHeroSubtitle || 'Where Parisian haute couture heritage meets modern architectural power. Crafted for visionaries who command prestige.'}
          </p>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-6">
            ...
            <h2 className="font-serif text-3xl sm:text-4xl text-neutral-100 font-light leading-tight">
              {settings.aboutPhilosophyTitle || 'Individually Sculpted to Accentuate Grace, Power, and Prestige'}
            </h2>
            ...
            <p className="text-xs sm:text-sm text-neutral-400 leading-relaxed font-light">
              {settings.aboutPhilosophyBody || 'Every garment carrying the Annex Couture signature undergoes hundreds of hours of precision hand-stitching, structural boning, and intricate beadwork. We do not mass-produce; we sculpt wearable art designed to endure through generations.'}
            </p>
        ...
        <div className="bg-gradient-to-r from-noir-900 via-noir-900 to-gold-950/20 rounded-3xl border border-gold-600/30 p-8 sm:p-12 lg:p-16 flex flex-col lg:flex-row items-center gap-12 shadow-2xl">
          ...
          <div className="space-y-6 flex-1 text-center lg:text-left">
            ...
            <p className="text-xs text-neutral-400 leading-relaxed font-light">
              Under {settings.founderName || 'Ayoola'}’s creative leadership, {settings.companyName || 'Annex Couture'} has redefined red carpet luxury, providing bespoke evening gowns, double-breasted suits, and one-of-a-kind gala masterpieces for discerning clientele across North America, Europe, and Africa.
            </p>
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {pillars.map((pillar, idx) => (
            <div key={pillar.title || idx} className="p-8 rounded-2xl bg-noir-900 border border-gold-600/20 space-y-4 text-center">
              <div className="w-12 h-12 rounded-full bg-gold-600/10 border border-gold-500/30 flex items-center justify-center mx-auto text-gold-400">
                {idx === 0 ? <Award className="w-6 h-6" /> : idx === 1 ? <Shield className="w-6 h-6" /> : <Crown className="w-6 h-6" />}
              </div>
              <h3 className="font-serif text-lg text-neutral-100">{pillar.title}</h3>
